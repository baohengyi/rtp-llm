...
BlockIdxType seedSingleTypeLowerTier(BlockTreeCache& cache, Tier source_tier, CacheKeyType key) {
    ...
    const auto  source_block = group->allocateSingleBlock(source_tier, BlockTreeRefType::CACHE);
...
// initMalloc must let cache matching/materialization run before classifying gross capacity because
// a lower cache tier may satisfy part of the request. If materialization fails, it must classify
// against the *failure-time* capacity snapshot before rollback; freeing first would make capacity
// look sufficient and downgrade a retryable race to an internal error. The InSequence expectations
...
// FullKVCacheGroup::match moved into the tree-backed allocator. Retain its
// original zero/partial/full hit assertions, including token lengths and IDs.
TEST_F(SingleTypeKVCacheAllocatorTest, FullGroupLegacyMatchCountsLengthsAndBlocks) {
    struct Scenario {
        CacheKeysType seed_keys;
        CacheKeysType query_keys;
        BlockIndicesType expected_blocks;
    };
    const std::vector<Scenario> scenarios = {
        {{101, 102}, {103, 104, 105, 106}, {}},
        {{101, 102}, {101, 102, 103, 1046}, {1, 2}},
        {{101, 102, 103, 104}, {101, 102, 103, 104}, {1, 2, 3, 4}},
    };
    for (const auto& scenario : scenarios) {
        const auto config = createSingleTypeTestConfig(1, 10, 4);
        allocator_ = std::make_shared<TestSingleTypeKVCacheAllocator>(config);
        ASSERT_TRUE(allocator_->init());

        auto seed = createBatchKVCacheResource(1, config);
        seed->setBatchCacheKeys(0, scenario.seed_keys);
        auto seed_tokens = createCompleteTokenIds(1, scenario.seed_keys.size() * 4, 4);
        ...
        size_t resident_prefix_length = 0;
        allocator_->insertIntoCache(InsertInfo{seed, seed_tokens, false}, resident_prefix_length);
        allocator_->free(FreeInfo{seed, seed_tokens});

        auto query = createBatchKVCacheResource(1, config);
        auto query_keys = scenario.query_keys;
        // The allocator also needs a fresh final page for inference. An unmatched
        // suffix leaves all four original match keys eligible for prefix reuse.
        query_keys.push_back(9999);
        query->setBatchCacheKeys(0, query_keys);
        auto query_tokens = createCompleteTokenIds(1, 20, 4);
        MallocInfo query_malloc{query, query_tokens};
        query_malloc.enable_cache_lookup = true;
        const auto result = allocator_->malloc(query_malloc);
        ASSERT_TRUE(result.success);
        EXPECT_EQ(query->cacheResource(0).reuseBlockNum(), scenario.expected_blocks.size());
        EXPECT_EQ(result.reuse_len, scenario.expected_blocks.size() * 4);
        const auto& blocks = query->blocks(0, 0);
        ASSERT_GE(blocks.size(), scenario.expected_blocks.size());
        EXPECT_EQ(BlockIndicesType(blocks.begin(), blocks.begin() + scenario.expected_blocks.size()),
                  scenario.expected_blocks);
        allocator_->free(FreeInfo{query, query_tokens});
    }
}
    ...
    for (const Tier source_tier : {Tier::HOST, Tier::DISK}) {
        ...
        coordinator->abort_callback_ = [&](LoadAsyncContext& context) {
            ...
            const size_t source_ref_after_abort = source_tier == Tier::HOST ?
                                                      group->hostPool()->treeRefCount(source_block) :
                                                      group->diskPool()->treeRefCount(source_block);
...
TEST_F(SingleTypeKVCacheAllocatorTest, UpdateKVBlockReleasesSharedBlocksFromEachDroppedBatch) {
    ...
    for (int batch_id = 0; batch_id < 3; ++batch_id) {
        block_pool->incRef(*shared_blocks);
    }
    ...
    block_pool->decRef(*shared_blocks);
}
