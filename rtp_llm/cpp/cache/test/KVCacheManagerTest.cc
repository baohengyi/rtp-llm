...
TEST_F(KVCacheManagerTest, Init_Throws_WhenHostCacheEnabledButSyncTimeoutInvalid) {
    auto          cache_config = makeSimpleMhaCacheConfig(1, 4, 2, rtp_llm::DataType::TYPE_INT8);
    for (int64_t timeout_ms : {int64_t{0}, int64_t{-1}, static_cast<int64_t>(std::numeric_limits<int>::max()) + 1}) {
        SCOPED_TRACE(timeout_ms);
        KVCacheConfig kv_cache_config;
        kv_cache_config.enable_memory_cache          = true;
        kv_cache_config.reuse_cache                  = true;
        kv_cache_config.memory_cache_size_mb         = 10;
        kv_cache_config.memory_cache_sync_timeout_ms = timeout_ms;

        auto kv_cache_manager = std::make_shared<KVCacheManager>(cache_config, false, nullptr, kv_cache_config);
        EXPECT_THROW(kv_cache_manager->init(), std::runtime_error);
        EXPECT_EQ(kv_cache_manager->allocator_, nullptr);
        EXPECT_EQ(kv_cache_manager->blockTreeCache(), nullptr);
        EXPECT_FALSE(kv_cache_manager->metrics_reporter_thread_.joinable());
    }
}

TEST_F(KVCacheManagerTest, InitRejectsInvalidDiskTimeoutBeforeAllocation) {
    const auto cache_config = makeSimpleMhaCacheConfig(1, 4, 2, DataType::TYPE_INT8);
    for (int64_t timeout_ms : {int64_t{0}, int64_t{-1}, static_cast<int64_t>(std::numeric_limits<int>::max()) + 1}) {
        SCOPED_TRACE(timeout_ms);
        KVCacheConfig kv_cache_config;
        kv_cache_config.enable_memory_cache          = false;
        kv_cache_config.memory_cache_sync_timeout_ms = 17;
        kv_cache_config.enable_disk_cache            = true;
        kv_cache_config.disk_cache_size_mb           = 1;
        kv_cache_config.disk_cache_sync_timeout_ms   = timeout_ms;
        // No path is needed: invalid timeout must fail before pool allocation
        // or mount validation, with this precise error rather than a path error.
        auto manager = std::make_shared<KVCacheManager>(cache_config, false, nullptr, kv_cache_config);
        ...
        try {
            ...
            timeout_threw = true;
            EXPECT_NE(std::string(error.what()).find("disk_cache_sync_timeout_ms"), std::string::npos);
        }
        EXPECT_TRUE(timeout_threw);
...
TEST_F(KVCacheManagerTest, InitIgnoresDiskTimeoutWhenDiskCacheIsDisabled) {
    const auto cache_config = makeSimpleMhaCacheConfig(1, 4, 2, DataType::TYPE_INT8);
    for (int64_t timeout_ms : {int64_t{0}, int64_t{-1}, static_cast<int64_t>(std::numeric_limits<int>::max()) + 1}) {
        SCOPED_TRACE(timeout_ms);
        KVCacheConfig kv_cache_config;
        kv_cache_config.enable_memory_cache          = false;
        kv_cache_config.memory_cache_sync_timeout_ms = 17;
        kv_cache_config.enable_disk_cache            = false;
        kv_cache_config.disk_cache_sync_timeout_ms   = timeout_ms;
        auto manager = std::make_shared<KVCacheManager>(cache_config, false, nullptr, kv_cache_config);
        ASSERT_TRUE(manager->init());
        ...
        EXPECT_EQ(cache->config().host_cache_sync_timeout_ms, 17);
        EXPECT_EQ(cache->config().disk_cache_sync_timeout_ms, 17);
...
class KVCacheConnectorCoordinatorTest: public KVCacheManagerTest {};

TEST_F(KVCacheConnectorCoordinatorTest, Init_ReturnFalse_WhenMemoryConfigInvalid) {
    // Keep the original coordinator's tiny configuration and invalid timeout.
    // KVCacheManager now owns the real allocator/factory initialization boundary.
    auto cache_config = makeSimpleMhaCacheConfig(
        1, 1, 4, DataType::TYPE_FP16, /*local_head_num_kv=*/1, /*size_per_head=*/1);
    cache_config.block_size_bytes = 1;
    KVCacheConfig kv_cache_config;
    kv_cache_config.enable_memory_cache          = true;
    kv_cache_config.reuse_cache                  = true;
    kv_cache_config.memory_cache_size_mb         = 1;
    kv_cache_config.memory_cache_sync_timeout_ms = 0;
    auto manager = std::make_shared<KVCacheManager>(cache_config, false, nullptr, kv_cache_config);
    ...
    try {
        ...
        timeout_threw = true;
        EXPECT_NE(std::string(error.what()).find("memory_cache_sync_timeout_ms"), std::string::npos);
    }
    EXPECT_TRUE(timeout_threw);
    // Validate the transfer configuration before any pool can mask its error.
    EXPECT_EQ(manager->allocator_, nullptr);
    EXPECT_EQ(manager->blockTreeCache(), nullptr);
    EXPECT_FALSE(manager->metrics_reporter_thread_.joinable());
}
