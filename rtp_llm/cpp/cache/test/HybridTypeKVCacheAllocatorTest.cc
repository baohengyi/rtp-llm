...
TEST_F(HybridTypeKVCacheAllocatorTest, IncrDecrKVCacheRefReferencesOnlyMatchedValidBlocksAcrossGroups) {
    ...
    resource.mutableBlockIds(/*group_id=*/0).assign(BlockIndicesType{blocks[0], NULL_BLOCK_IDX, blocks[1]});
    resource.mutableBlockIds(/*group_id=*/1).assign(BlockIndicesType{blocks[2], blocks[3], NULL_BLOCK_IDX});
...
TEST_F(HybridTypeKVCacheAllocatorTest, ConnectorRefPreservesDummyTailAcrossGroupsAndReleasesValidBlocks) {
    ...
    resource.mutableBlockIds(/*group_id=*/0).assign(BlockIndicesType{blocks[0], NULL_BLOCK_IDX, NULL_BLOCK_IDX});
    resource.mutableBlockIds(/*group_id=*/1).assign(BlockIndicesType{NULL_BLOCK_IDX, blocks[1], NULL_BLOCK_IDX});
    ...
    EXPECT_EQ(ref->blocks(0), (BlockIndicesType{blocks[0], NULL_BLOCK_IDX, NULL_BLOCK_IDX}));
    EXPECT_EQ(ref->blocks(1), (BlockIndicesType{NULL_BLOCK_IDX, blocks[1], NULL_BLOCK_IDX}));
    EXPECT_EQ(block_pool->refCount(blocks[0]), 2u);
    EXPECT_EQ(block_pool->refCount(blocks[1]), 2u);

    block_pool->decRef(blocks);
    EXPECT_EQ(block_pool->referencedBlocksNum(), 2u);
    EXPECT_EQ(allocator->freeBlocksNum(), free_before - 2);

    ref.reset();
    EXPECT_EQ(block_pool->referencedBlocksNum(), 0u);
    EXPECT_EQ(allocator->freeBlocksNum(), free_before);
}
...
// The old LinearKVCacheGroup cache methods now belong to the allocator/tree.
// A valid hybrid topology supplies the FULL group alongside the LINEAR state.
TEST_F(HybridTypeKVCacheAllocatorTest, LegacyLinearPutIntoCacheSkipsNullBlocks) {
    ...
    ASSERT_TRUE(allocated.has_value());
    const auto& blocks = *allocated;
    pool->incRef(blocks);
    auto resource = makeBatchResource(1, config, {100, 101, 102, 103});
    resource->mutableBlockIds(0, 0).assign({NULL_BLOCK_IDX, blocks[0], NULL_BLOCK_IDX, blocks[1]});
    resource->mutableBlockIds(0, 1).assign({blocks[2], blocks[3], blocks[4], blocks[5]});
    size_t resident_prefix_length = 0;
    allocator->insertIntoCache(InsertInfo{resource, nullptr, false}, resident_prefix_length);
    const auto path = allocator->blockTreeCacheOwner()->tree()->findNode({100, 101, 102, 103});
    ASSERT_EQ(path.size(), 4u);
    EXPECT_TRUE(path[0]->group_set_resources[0].is_empty());
    EXPECT_EQ(path[1]->group_set_resources[0].device_blocks, (BlockIndicesType{blocks[0]}));
    EXPECT_TRUE(path[2]->group_set_resources[0].is_empty());
    EXPECT_EQ(path[3]->group_set_resources[0].device_blocks, (BlockIndicesType{blocks[1]}));
    pool->decRef(blocks);
}

TEST_F(HybridTypeKVCacheAllocatorTest, LegacyLinearMatchSingleKeyReturnsMatchedBlockOrEmpty) {
    const auto config = makeTinyHybridConfig();
    auto allocator = std::make_shared<TestHybridTypeKVCacheAllocator>(config, AllocationType::DEVICE);
    ASSERT_TRUE(allocator->init());
    auto pool = allocator->getDeviceBlockPool();
    ...
    const auto& blocks = *allocated;
    pool->incRef(blocks);
    auto resource = makeBatchResource(1, config, {123});
    resource->mutableBlockIds(0, 0).assign({blocks[0]});
    resource->mutableBlockIds(0, 1).assign({blocks[1]});
    size_t resident_prefix_length = 0;
    allocator->insertIntoCache(InsertInfo{resource, nullptr, false}, resident_prefix_length);
    const auto& cache = allocator->blockTreeCacheOwner();
    auto hit = cache->match({123});
    EXPECT_EQ(cache->matchedBlocksForGroup(0, hit.matched_device_resources), (BlockIndicesType{blocks[0]}));
    block_tree_cache_test::releaseRequestRefsForTest(*cache, hit.matched_device_resources);
    auto miss = cache->match({999});
    EXPECT_TRUE(cache->matchedBlocksForGroup(0, miss.matched_device_resources).empty());
    block_tree_cache_test::releaseRequestRefsForTest(*cache, miss.matched_device_resources);
    pool->decRef(blocks);
}

TEST_F(HybridTypeKVCacheAllocatorTest, LegacyLinearMallocEnsuresFreeBlocksByEvictingCache) {
    const auto config = makeTinyHybridConfig();
    auto allocator = std::make_shared<TestHybridTypeKVCacheAllocator>(config, AllocationType::DEVICE);
    ASSERT_TRUE(allocator->init());
    allocator->setReserveBlocksNum(0);
    auto pool = allocator->getDeviceBlockPool();
    ASSERT_EQ(pool->freeBlocksNum(), 9u);
    const auto cached = pool->malloc(2);
    ASSERT_TRUE(cached.has_value());
    pool->incRef(*cached);
    auto seed = makeBatchResource(1, config, {123});
    seed->mutableBlockIds(0, 0).assign({(*cached)[0]});
    seed->mutableBlockIds(0, 1).assign({(*cached)[1]});
    size_t resident_prefix_length = 0;
    allocator->insertIntoCache(InsertInfo{seed, nullptr, false}, resident_prefix_length);
    pool->decRef(*cached);
    const auto occupied = pool->malloc(pool->freeBlocksNum());
    ASSERT_TRUE(occupied.has_value());
    pool->incRef(*occupied);
    ASSERT_EQ(pool->freeBlocksNum(), 0u);

    auto resource = makeBatchResource(1, config, {999});
    auto tokens = makeCompleteTokenIds(1, 4, 4);
    MallocInfo request{resource, tokens};
    request.enable_cache_lookup = false;
    request.reuse_cache = false;
    ASSERT_TRUE(allocator->malloc(request).success);
    ASSERT_EQ(resource->blocks(0, 0).size(), 1u);
    EXPECT_FALSE(isNullBlockIdx(resource->blocks(0, 0).front()));
    allocator->free(FreeInfo{resource, tokens});
    pool->decRef(*occupied);
    EXPECT_EQ(pool->freeBlocksNum(), 9u);
}

TEST_F(HybridTypeKVCacheAllocatorTest, LegacyLinearInsertIntoCacheWithEmptyInputsIsNoop) {
    const auto config = makeTinyHybridConfig();
    auto allocator = std::make_shared<TestHybridTypeKVCacheAllocator>(config, AllocationType::DEVICE);
    ASSERT_TRUE(allocator->init());
    auto pool = allocator->getDeviceBlockPool();
    ...
    auto empty_keys = makeBatchResource(1, config, {});
    empty_keys->mutableBlockIds(0, 0).assign(*allocated);
    empty_keys->mutableBlockIds(0, 1).assign(*allocated);
    size_t resident_prefix_length = 0;
    const auto& cache = allocator->blockTreeCacheOwner();
    ASSERT_EQ(cache->tree()->size(), 0u);
    allocator->insertIntoCache(InsertInfo{empty_keys, nullptr, false}, resident_prefix_length);
    EXPECT_EQ(cache->tree()->size(), 0u);
    auto empty_blocks = makeBatchResource(1, config, {100, 101});
    allocator->insertIntoCache(InsertInfo{empty_blocks, nullptr, false}, resident_prefix_length);
    EXPECT_EQ(cache->tree()->size(), 0u);
    EXPECT_EQ(pool->freeBlocksNum(), 7u);
