...
pytestmark = [pytest.mark.manual, pytest.mark.smoke, pytest.mark.gpu(type="H20", count=1)]
import unittest
...
class BenchmarkSmokeTest(unittest.TestCase):
    def test_smoke_suite(self):
        driver = str(Path(__file__).with_name("run_block_tree_cache_benchmark.py"))
        self.assertTrue(Path(driver).is_file(), "packaged benchmark driver missing")
