...
def parse_args():
    ...
    parser.add_argument(
        ...
        "(default: use the installed package)",
    )
    parser.add_argument(
        "--model-profile",
        default=None,
        help="Path to model profile JSON " "(default: use the installed package)",
    )
    parser.add_argument(
        "--output-dir",
...
def run_suite(
    ...
    for case in cases:
        ...
        case_payloads = model_profile_payloads
...
def resolve_runfile_path(name: str) -> Optional[str]:
    """Resolve packaged benchmark data or the staged native executable."""
    here = Path(__file__).resolve().parent
    candidate = here / name
    if candidate.is_file():
        return str(candidate)
    if name == "block_tree_cache_gpu_benchmark":
        binary = here.parents[3] / "libs" / "benchmark" / name
        if binary.is_file():
            return str(binary)
    return None
...
def main():
    ...
    # Resolve binary path (explicit arg wins, else package-relative lookup)
    binary = args.binary
    if binary is None:
