...
TEST(MemoryDiskBlockCacheTest, ContainsAndMatchMemoryAndDisk) {
    auto host_pool = makeHostPool(1, 32);
    auto disk_pool = makeDiskPool(1, 32, std::make_unique<MemoryDiskBlockIO>());
    ASSERT_NE(host_pool, nullptr);
    ASSERT_NE(disk_pool, nullptr);

    // Keep the old physical slot identities: host block 10 and disk slot 20.
    const auto host_blocks = host_pool->malloc(10);
    const auto disk_blocks = disk_pool->malloc(20);
    ASSERT_TRUE(host_blocks.has_value());
    ASSERT_TRUE(disk_blocks.has_value());
    host_pool->incTreeRef(*host_blocks, BlockTreeRefType::LOAD);
    disk_pool->incTreeRef(*disk_blocks, BlockTreeRefType::LOAD);
    block_tree_cache_detail::ScopeRollback release_seed_refs([&] {
        host_pool->decTreeRef(*host_blocks, BlockTreeRefType::LOAD);
        disk_pool->decTreeRef(*disk_blocks, BlockTreeRefType::LOAD);
    });
    ASSERT_TRUE(host_pool->isAllocated(10));
    ASSERT_TRUE(disk_pool->isAllocated(20));
    ...
    GroupSetResource memory_resource;
    memory_resource.host_block = 10;
    host_pool->incTreeRef(10, BlockTreeRefType::CACHE);
    ASSERT_TRUE(insertGroupSetResources(*cache, {1}, {{memory_resource}}));
    GroupSetResource disk_resource;
    disk_resource.disk_block = 20;
    disk_pool->incTreeRef(20, BlockTreeRefType::CACHE);
    ASSERT_TRUE(insertGroupSetResources(*cache, {2}, {{disk_resource}}));

    ASSERT_EQ(cache->tree()->findNode({1}).size(), 1u);
    ASSERT_EQ(cache->tree()->findNode({2}).size(), 1u);
    const auto expect_match = [&](CacheKeyType key, Tier tier, BlockIdxType block) {
        auto result = cache->match({key});
        auto context = takeLoadContext(result);
        ASSERT_NE(context, nullptr);
        EXPECT_EQ(context->matchedBlocks(), 1u);
        ASSERT_EQ(context->loadDescs().size(), 1u);
        EXPECT_EQ(context->loadDescs()[0].source_tier, tier);
        EXPECT_EQ(context->loadDescs()[0].source_blocks, (BlockIndicesType{block}));
        context.reset();
        releaseRequestRefsForTest(*cache, result.matched_device_resources);
        BlockTreeCacheTestPeer::waitForTaskPoolIdleForTest(*cache);
    };
    expect_match(1, Tier::HOST, 10);
    expect_match(2, Tier::DISK, 20);
}
