...
TEST_F(DeviceLinearKVCacheGroupTest, DefaultPolicyDrivesBehaviorInterfaces) {
    ...
    DeviceLinearKVCacheGroup disabled_group({}, spec, block_pool, 0, 2, disabled_policy);
...
TEST_F(DeviceLinearKVCacheGroupTest, ActiveTailPolicyMatchesInitialAllocationAndPeakEstimate) {
    for (const uint32_t configured_tail_blocks : {0u, 1u, 2u, 4u}) {
        ...
        auto               spec   = makeLinearSpec(/*seq_size_per_block=*/4);
        DeviceLinearKVCacheGroup group(/*layer_ids=*/{},
                                 spec,
                                 block_pool,
                                 /*group_id=*/0,
                                 /*linear_step=*/8,
...
TEST_F(DeviceLinearKVCacheGroupTest, EstimatePeakContinuesCleanupAcrossSparseHoles) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());

    auto               spec = makeLinearSpec(/*seq_size_per_block=*/4);
    DeviceLinearKVCacheGroup group(/*layer_ids=*/{}, spec, block_pool, /*group_id=*/0, /*linear_step=*/8);
    ASSERT_TRUE(group.init());
...
TEST_F(DeviceLinearKVCacheGroupTest, ActiveTailPolicyDrivesInitialBatchPeak) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
...
TEST_F(DeviceLinearKVCacheGroupTest, EstimatePeakNeedBlocksIncludesTransientTailAllocation) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
...
TEST_F(DeviceLinearKVCacheGroupTest, EstimateInitialBatchPeakKeepsSharedAndPrivateTailsDistinct) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
...
TEST_F(DeviceLinearKVCacheGroupTest, EstimatePeakNeedBlocksAddsTransientWhenFreshResourceCrossesTwoBoundaries) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
...
TEST_F(DeviceLinearKVCacheGroupTest, EstimatePeakNeedBlocksIncludesTransientReserveAllocation) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
