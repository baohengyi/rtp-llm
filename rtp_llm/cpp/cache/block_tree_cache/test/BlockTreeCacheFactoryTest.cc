...
// Exact legacy seven-group test layout. Keep byte strides, tags, layer
// membership and the hca_state reuse exclusion supplied by makeDsv4Desc.
inline constexpr size_t kLegacyDsv4PoolNum = 7;

struct LegacyDsv4TypedPool {
    const char* tag;
    const char* kind;
    size_t      stride_bytes;
};

const std::vector<LegacyDsv4TypedPool>& legacyDsv4TypedPools() {
    static const std::vector<LegacyDsv4TypedPool> kPools = {
...
const std::vector<std::vector<std::string>>& legacyDsv4TypedLayerTags() {
...
CacheConfig makeLegacyDsv4TypedConnectorConfig() {
    CacheConfig config;
    config.layer_num                   = 2;
    ...
    const auto& pools      = legacyDsv4TypedPools();
    const auto& layer_tags = legacyDsv4TypedLayerTags();
    ...
    specs.reserve(kLegacyDsv4PoolNum);
    layers_by_group.reserve(kLegacyDsv4PoolNum);
    types.reserve(kLegacyDsv4PoolNum);
    tags.reserve(kLegacyDsv4PoolNum);
    policies.reserve(kLegacyDsv4PoolNum);
    kv_strides.reserve(kLegacyDsv4PoolNum);

    for (const auto& pool : pools) {
        const auto desc   = test::makeDsv4Desc(pool.tag, pool.kind, /*entry_elems=*/1, rtp_llm::DataType::TYPE_UINT8);
        ...
        specs.push_back(test::makeResolvedOpaqueSpec(desc.is_state_cache,
                                               pool.tag,
    ...
    config.setGroupBlockLayout(std::vector<uint32_t>(kLegacyDsv4PoolNum, config.block_num),
                               kv_strides,
                               std::vector<size_t>(kLegacyDsv4PoolNum, 0));
...
insertOneKeyThroughAllocator(const CacheConfig& config, const KVCacheAllocatorPtr& allocator, CacheKeyType key) {
    ...
    }
    {
        ...
        allocator->insertIntoCache(InsertInfo{resource, nullptr, /*is_resident=*/false}, resident_prefix_length);
    }
    return blocks;
}
...
class KVCacheMemoryConnectorTest: public DeviceTestBase {};

class KVCacheMemoryConnectorDualPoolTest: public DeviceTestBase {};
...
TEST_F(BlockTreeCacheFactoryTest, ProductionEvictionConfigurationPropagatesToBlockTreeCache) {
    ...
    kv_cache_config.memory_eviction_policy = "lfu";
...
TEST_F(BlockTreeCacheFactoryTest, UnsupportedProductionEvictionPolicyFailsClosed) {
    ...
    const std::vector<std::pair<const char*, PolicyField>> policy_fields = {
        ...
        {"memory", &KVCacheConfig::memory_eviction_policy},
...
TEST_F(BlockTreeCacheFactoryTest, TierWatermarksPropagateAndInvalidCombinationsAreRejected) {
    ...
    kv_cache_config.block_tree_memory_evict_low_watermark_ratio  = 0.62;
    kv_cache_config.block_tree_memory_evict_high_watermark_ratio = 0.72;
...
TEST_F(KVCacheMemoryConnectorTest, asyncMatch_ReturnNull_WhenNoPrefixMatched) {
    const auto config = test::makeSimpleMhaCacheConfig(
        4, 10, 8, DataType::TYPE_FP16, /*local_head_num_kv=*/8, /*size_per_head=*/128);
    auto allocator = initAllocator<SingleTypeKVCacheAllocator>(config);
    KVCacheConfig kv_config;
    kv_config.enable_memory_cache          = true;
    kv_config.enable_remote_cache          = false;
    kv_config.memory_cache_size_mb         = 64;
    kv_config.memory_cache_sync_timeout_ms = 1000;
    auto cache = createBlockTreeCache(config, kv_config, allocator);
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isHostCacheEnabled());
    ASSERT_FALSE(cache->isRemoteCacheEnabled());
    ASSERT_EQ(cache->tree()->size(), 0u);
    ASSERT_EQ(cache->groupSets().size(), 1u);
    const auto host_pool = cache->groupSets()[0]->hostPool();
    ASSERT_NE(host_pool, nullptr);
    const auto free_host_before = host_pool->freeBlocksNum();
    const auto free_device_before = allocator->getDeviceBlockPool()->freeBlocksNum();

    // Matching now precedes target allocation and accepts keys directly. Keep
    // the original zero-reuse, empty HOST-cache query and null-context contract.
    const CacheKeysType cache_keys{71001, 71002};
    const auto result = cache->match(cache_keys);
    EXPECT_EQ(result.async_context, nullptr);
    EXPECT_EQ(result.matched_device_blocks, 0u);
    EXPECT_TRUE(result.matched_device_resources.empty());
    EXPECT_EQ(cache->tree()->size(), 0u);
    EXPECT_EQ(host_pool->freeBlocksNum(), free_host_before);
    EXPECT_EQ(allocator->getDeviceBlockPool()->freeBlocksNum(), free_device_before);
}
...
TEST_F(KVCacheMemoryConnectorTest, init_ReturnFalse_WhenMemoryCacheSyncTimeoutMsZero) {
    ...
    kv_config.enable_memory_cache          = true;
    kv_config.memory_cache_size_mb         = 64;
    kv_config.memory_cache_sync_timeout_ms = 0;

    const auto device_pool = allocator->getDeviceBlockPool();
    ASSERT_NE(device_pool, nullptr);
    const auto free_blocks_before = device_pool->freeBlocksNum();
    BlockTreeCachePtr cache;
    // The factory owns the tree, transfer dispatcher and task pool that replace
    // the old connector's cache, broadcast manager and wait-done pool. Invalid
    // initialization must throw, not return a usable or partially built cache.
    EXPECT_THROW(cache = createBlockTreeCache(config, kv_config, allocator), std::runtime_error);
    EXPECT_EQ(cache, nullptr);
    EXPECT_EQ(allocator->blockTreeCache(), nullptr);
    EXPECT_EQ(device_pool->freeBlocksNum(), free_blocks_before);
}

TEST_F(KVCacheMemoryConnectorTest, initBlockPool_PrefixPoolsDefaultKeepEqualKeyCapacity) {
    const auto config = makeLegacyDsv4TypedConnectorConfig();
    auto allocator = initAllocator<HybridPoolKVCacheAllocator>(config);
    KVCacheConfig kv_config;
    kv_config.enable_memory_cache          = true;
    ...
    BlockTreeCachePtr cache;
    ASSERT_NO_THROW(cache = createBlockTreeCache(config, kv_config, allocator));
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isInitialized());
    ASSERT_EQ(cache->groupSets().size(), 2u);
    const auto& compressed = cache->groupSets()[0];
    const auto& state_swa = cache->groupSets()[1];
    ASSERT_EQ(compressed->groupType(), CacheGroupType::FULL);
    ASSERT_EQ(state_swa->groupType(), CacheGroupType::SWA);
    EXPECT_EQ(compressed->groupIds(), (std::vector<size_t>{0, 1, 2}));
    EXPECT_EQ(state_swa->groupIds(), (std::vector<size_t>{3, 4, 6}));
    ASSERT_NE(compressed->hostPool(), nullptr);
    ASSERT_NE(state_swa->hostPool(), nullptr);
    EXPECT_EQ(compressed->payloadBytes(), 72u);
    EXPECT_EQ(state_swa->payloadBytes(), 76u);
    EXPECT_GT(compressed->hostPool()->totalBlocksNum(), 0u);
    EXPECT_EQ(compressed->hostPool()->totalBlocksNum(), state_swa->hostPool()->totalBlocksNum());
}

TEST_F(KVCacheMemoryConnectorTest, initDiskBlockPool_PrefixTreeCreatesTypedDiskPools) {
    const auto config = makeLegacyDsv4TypedConnectorConfig();
    auto allocator = initAllocator<HybridPoolKVCacheAllocator>(config);
    block_transfer_engine_test::TempDirGuard disk0("legacy_dsv4_typed_pools");
    ASSERT_FALSE(disk0.path.empty());
    KVCacheConfig kv_config;
    kv_config.enable_memory_cache          = true;
    kv_config.memory_cache_size_mb         = 64;
    kv_config.memory_cache_sync_timeout_ms = 1000;
    kv_config.enable_disk_cache            = true;
    kv_config.disk_cache_paths             = disk0.path;
    kv_config.disk_cache_size_mb           = 1;
    kv_config.disk_cache_buffered_io       = true;
    kv_config.disk_cache_sync_timeout_ms   = 1000;

    auto cache = createBlockTreeCache(config, kv_config, allocator, ParallelismConfig{});
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isInitialized());
    ASSERT_TRUE(cache->isHostCacheEnabled());
    ASSERT_TRUE(cache->isDiskCacheEnabled());
    ASSERT_EQ(cache->groupSets().size(), 2u);
    const auto& compressed = cache->groupSets()[0];
    const auto& state_swa = cache->groupSets()[1];
    ASSERT_EQ(compressed->groupType(), CacheGroupType::FULL);
    ...
    EXPECT_EQ(compressed->groupIds(), (std::vector<size_t>{0, 1, 2}));
    EXPECT_EQ(state_swa->groupIds(), (std::vector<size_t>{3, 4, 6}));
    ASSERT_NE(compressed->diskPool(), nullptr);
    ASSERT_NE(state_swa->diskPool(), nullptr);
    EXPECT_NE(compressed->diskPool(), state_swa->diskPool());
    EXPECT_GT(compressed->diskPool()->totalBlocksNum(), 0u);
    EXPECT_GT(state_swa->diskPool()->totalBlocksNum(), 0u);
    EXPECT_EQ(compressed->diskPool()->payloadBytes(), 72u);
    EXPECT_EQ(state_swa->diskPool()->payloadBytes(), 76u);
}
...
TEST_F(KVCacheMemoryConnectorTest, initBlockPool_ReturnTrue_AndRegistersPool) {
    ...
    ASSERT_TRUE(cache->isInitialized());
    ASSERT_EQ(cache->groupSets().size(), 1u);
    ASSERT_NE(cache->groupSets()[0]->hostPool(), nullptr);
    EXPECT_GT(cache->groupSets()[0]->hostPool()->totalBlocksNum(), 0u);
}

TEST_F(KVCacheMemoryConnectorTest, init_ReturnTrue_WithWorkerAddrs) {
    // Preserve the original four live local RPC workers. Initialization does
    // not issue transfers, so the generated service needs no request overrides.
    struct Worker {
    ...
    for (size_t index = 0; index < 4; ++index) {
        ...
        builder.RegisterService(&worker->service);
        worker->server = builder.BuildAndStart();
        ASSERT_NE(worker->server, nullptr);
        ASSERT_GT(port, 0);
        worker_addrs.push_back("127.0.0.1:" + std::to_string(port));
    ...
    kv_config.memory_cache_size_mb         = 64;
    kv_config.memory_cache_sync_timeout_ms = 1000;
    auto broadcast_manager = std::make_shared<BroadcastManager>(worker_addrs);
    ASSERT_TRUE(broadcast_manager->init());

    auto cache = createBlockTreeCache(config, kv_config, allocator, {}, nullptr, broadcast_manager);
    ASSERT_NE(cache, nullptr);
    EXPECT_TRUE(cache->isInitialized());
    ASSERT_NE(cache->tree(), nullptr);
    ASSERT_NE(cache->transfer_dispatcher_, nullptr);
    const auto& multi_rank_engine = cache->transfer_dispatcher_->multi_rank_engine_;
    ASSERT_NE(multi_rank_engine, nullptr);
    ASSERT_NE(multi_rank_engine->broadcast_manager_, nullptr);
    EXPECT_EQ(multi_rank_engine->broadcast_manager_, broadcast_manager);
    EXPECT_EQ(multi_rank_engine->broadcast_manager_->workerNum(), worker_addrs.size());
    EXPECT_EQ(multi_rank_engine->broadcast_manager_->workerNum(), 4u);
}
...
TEST_F(KVCacheMemoryConnectorDualPoolTest, Init_PureFullUsesSinglePool) {
    ...
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isInitialized());
    // The old complete/incomplete branches are represented by GroupSets now;
    // a pure FULL model must still have exactly one registered HOST pool.
    ASSERT_EQ(cache->groupSets().size(), 1u);
    const auto& group = cache->groupSets()[0];
    EXPECT_EQ(group->groupType(), CacheGroupType::FULL);
    EXPECT_EQ(group->groupIds(), (std::vector<size_t>{0}));
    EXPECT_NE(group->hostPool(), nullptr);
...
TEST_F(KVCacheMemoryConnectorTest, initDiskBlockPool_UsesLocalRankPathAndPreallocatesFile) {
    const auto config = test::makeSimpleMhaCacheConfig(
        4, 10, 8, DataType::TYPE_FP16, /*local_head_num_kv=*/8, /*size_per_head=*/128);
    auto allocator = initAllocator<SingleTypeKVCacheAllocator>(config);
    block_transfer_engine_test::TempDirGuard disk0("legacy_memory_connector_rank0");
    block_transfer_engine_test::TempDirGuard disk1("legacy_memory_connector_rank1");
    ASSERT_FALSE(disk0.path.empty());
    ...
    kv_config.memory_cache_sync_timeout_ms = 1000;
    kv_config.enable_disk_cache            = true;
    kv_config.disk_cache_paths             = disk0.path + "," + disk1.path;
    kv_config.disk_cache_size_mb           = 1;
    kv_config.disk_cache_buffered_io       = true;
    kv_config.disk_cache_sync_timeout_ms   = 1000;
    ParallelismConfig parallelism;
    parallelism.world_size       = 2;
    parallelism.world_rank       = 5;
    parallelism.local_world_size = 2;
    parallelism.local_rank       = 1;

    auto cache = createBlockTreeCache(config, kv_config, allocator, parallelism);
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isInitialized());
    ASSERT_EQ(cache->groupSets().size(), 1u);
    EXPECT_EQ(cache->groupSets()[0]->groupType(), CacheGroupType::FULL);
    const auto pool = cache->groupSets()[0]->diskPool();
    ASSERT_NE(pool, nullptr);
    // Preserve local-rank mount selection and rank isolation with the main
    // BlockTree disk-pool filename, replacing the removed complete.kv format.
    EXPECT_EQ(pool->filePath(),
              disk1.path + "/rtp_llm_disk_kv/disk_block_pool_block_tree_disk_full_r5_l1.bin");
    EXPECT_GT(pool->totalBlocksNum(), 0u);
    EXPECT_EQ(pool->freeBlocksNum(), pool->totalBlocksNum());
    struct stat st {};
    ASSERT_EQ(::stat(pool->filePath().c_str(), &st), 0);
    EXPECT_EQ(static_cast<size_t>(st.st_size), (pool->totalBlocksNum() + 1) * pool->strideBytes());
}

// Keep the legacy large MLA split-KV/scale regression on the real allocator and
// transfer engine. The old connector's direct copy entry point did not use RPC.
TEST_F(KVCacheMemoryConnectorTest, copyCache_ReturnTrue_H2D_SplitKvScale_NoBlockCopyOpt) {
    ...
    constexpr int      kCopyBlockCount   = (kCopySeqLen + kSeqPerBlock - 1) / kSeqPerBlock;
    ...
    // Start with the original fixture's config before its MLA overrides, including
    // the inherited kernel block size, so this does not silently change the layout.
    ...
    cache_config.seq_size_per_block    = kSeqPerBlock;
    cache_config.use_mla               = true;
    cache_config.is_sparse             = false;
    cache_config.dtype                 = DataType::TYPE_FP8_E4M3;
    cache_config.kv_block_stride_bytes = kKvBytesPerTok * kSeqPerBlock;
    cache_config.kv_scale_stride_bytes = kScaleBytesPerTok * kSeqPerBlock;
    cache_config.kv_block_size_bytes   = kLayerNum * cache_config.kv_block_stride_bytes;
    cache_config.kv_scale_size_bytes   = kLayerNum * cache_config.kv_scale_stride_bytes;
    cache_config.block_size_bytes      = cache_config.kv_block_size_bytes + cache_config.kv_scale_size_bytes;
    const size_t per_layer_bytes       = cache_config.kv_block_stride_bytes + cache_config.kv_scale_stride_bytes;
    cache_config.layer_to_block_stride_bytes.assign(kLayerNum, static_cast<int>(per_layer_bytes));
    std::vector<int> layer_ids(kLayerNum);
    for (int layer = 0; layer < kLayerNum; ++layer) {
        layer_ids[layer] = layer;
    }
    cache_config.fromGroupedSpecs({mla_spec}, {layer_ids}, {CacheGroupType::FULL}, {"default"});
    cache_config.setGroupBlockLayout(
        {kBlockNum}, {cache_config.kv_block_stride_bytes}, {cache_config.kv_scale_stride_bytes});
    ASSERT_EQ(mla_spec->block_size_bytes(), cache_config.kv_block_stride_bytes);

    const size_t merged_one_key = kLayerNum * per_layer_bytes;
    const int pool_mb = static_cast<int>((merged_one_key * kCopyBlockCount + (1024ULL * 1024 - 1)) / (1024 * 1024))
                        + 256;
    KVCacheConfig kv_cache_config;
    kv_cache_config.enable_memory_cache          = true;
    kv_cache_config.memory_cache_size_mb         = std::max(pool_mb, 512);
    kv_cache_config.memory_cache_sync_timeout_ms = 1000;

    auto allocator = initAllocator<SingleTypeKVCacheAllocator>(cache_config);
    auto cache     = createBlockTreeCache(cache_config, kv_cache_config, allocator, ParallelismConfig{});
    ASSERT_NE(cache, nullptr);
    ASSERT_TRUE(cache->isInitialized());
    ASSERT_EQ(cache->groupSets().size(), 1u);
    const auto group = cache->groupSets()[0];
    ASSERT_EQ(group->groupIds(), (std::vector<size_t>{0}));
    ASSERT_EQ(group->devicePools().size(), 1u);
    ASSERT_EQ(group->payloadBytes(), merged_one_key);
    const auto device_pool = group->devicePools()[0];
    const auto host_pool   = group->hostPool();
    ASSERT_NE(device_pool, nullptr);
    ASSERT_NE(host_pool, nullptr);

    // Reserve the original physical GPU indices 2..80, including unused index 1.
    const auto device_blocks = device_pool->malloc(kBlockNum - 1);
    ASSERT_TRUE(device_blocks.has_value());
    ASSERT_EQ(device_blocks->size(), kBlockNum - 1);
    device_pool->incRef(*device_blocks);
    block_tree_cache_detail::ScopeRollback release_device([&]() {
        cache.reset();
        device_pool->decRef(*device_blocks);
    });
    const auto host_blocks = host_pool->malloc(kCopyBlockCount);
    ASSERT_TRUE(host_blocks.has_value());
    ASSERT_EQ(host_blocks->size(), kCopyBlockCount);
    host_pool->incTreeRef(*host_blocks, BlockTreeRefType::LOAD);
    block_tree_cache_detail::ScopeRollback release_host([&]() {
        cache.reset();
        host_pool->decTreeRef(*host_blocks, BlockTreeRefType::LOAD);
    });

    std::vector<TransferDescriptor> descriptors;
    descriptors.reserve(kCopyBlockCount);
    for (int i = 0; i < kCopyBlockCount; ++i) {
        ASSERT_TRUE(device_pool->isAllocated(kGpuBlockBase + i));
        const auto buffer = host_pool->blockBuffer((*host_blocks)[i]);
        ASSERT_NE(buffer.addr, nullptr);
        ASSERT_GE(buffer.payload_bytes, merged_one_key);
        const auto tag = static_cast<uint8_t>(0x21 + (i % 94));
        std::memset(buffer.addr, tag, merged_one_key);
        descriptors.push_back(
            TransferDescriptor::hostToDevice(group->groupSetId(), (*host_blocks)[i], {kGpuBlockBase + i}));
    }
    // executeTransfer waits for the actual copy and checks its completion status.
    ASSERT_TRUE(cache->executeTransfer(block_transfer_engine_test::makeTransferTask(std::move(descriptors))));

    for (int i = 0; i < kCopyBlockCount; ++i) {
        const auto tag = static_cast<uint8_t>(0x21 + (i % 94));
        for (int layer = 0; layer < kLayerNum; ++layer) {
            SCOPED_TRACE(::testing::Message() << "block=" << kGpuBlockBase + i << " layer=" << layer);
            const auto buffers = allocator->convertIndexToBuffer(layer, kGpuBlockBase + i);
            ASSERT_GE(buffers.size(), 2u);
            size_t actual_bytes = 0;
            for (const auto& buffer : buffers) {
                ASSERT_NE(buffer.addr, nullptr);
                ASSERT_GT(buffer.size_bytes, 0u);
                actual_bytes += buffer.size_bytes;
                auto device = torch::from_blob(buffer.addr,
                                               {static_cast<int64_t>(buffer.size_bytes)},
                                               torch::TensorOptions(torch::kUInt8).device(torch::kCUDA));
                auto host = device.cpu();
                const auto* data = host.data_ptr<uint8_t>();
                const auto* mismatch =
                    std::find_if(data, data + buffer.size_bytes, [tag](uint8_t value) { return value != tag; });
                ASSERT_EQ(static_cast<size_t>(mismatch - data), buffer.size_bytes);
            }
            ASSERT_EQ(actual_bytes, per_layer_bytes);
        }
    }
}
