...
TEST(BlockTransferRequestConverterTest, EncodeRejectsExpiredTask) {
...
TEST(BlockTransferRequestConverterTest, RejectsExtraUnknownMemberEvenWhenAllRequiredMembersExist) {
    for (const auto& descriptor : {TransferDescriptor::deviceToHost(2, {11, 12}, 21),
        ...
        MemoryOperationRequestPB request;
        ASSERT_TRUE(BlockTransferRequestConverter::encodeTransfer(
            request, makeTransferTask({descriptor}), groupSets()));
        ASSERT_TRUE(decodeSingleTransfer(request).isExecutable());

        auto* extra = request.mutable_copy_items(0)->add_group_blocks();
        extra->set_group_id(99);
        extra->set_block_id(13);
        expectDecodeFailure(request);
    }
}

TEST(BlockTransferRequestConverterTest, RejectsExtraDuplicateMemberEvenWhenAllRequiredMembersExist) {
    for (const auto& descriptor : {TransferDescriptor::deviceToHost(2, {11, 12}, 21),
                                   TransferDescriptor::hostToDevice(2, 21, {11, 12}),
                                   TransferDescriptor::deviceToDisk(2, {11, 12}, 21),
                                   TransferDescriptor::diskToDevice(2, 21, {11, 12})}) {
        ...
        const auto duplicate = request.copy_items(0).group_blocks(0);
        request.mutable_copy_items(0)->add_group_blocks()->CopyFrom(duplicate);
        expectDecodeFailure(request);
    }
}
