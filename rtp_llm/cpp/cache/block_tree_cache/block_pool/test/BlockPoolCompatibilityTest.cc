...
protected:
    ...
    // New allocations start unowned; reproduce the old request-owned allocation
    // contract through the public pool API before checking lifetime assertions.
    BlockIdList allocateRequestBlocks(size_t count) {
        auto blocks = block_pool_->malloc(count);
        if (!blocks) {
            return {};
        }
    ...
    std::shared_ptr<DeviceBlockPool> block_pool_;
...
TEST_F(BlockPoolTest, ConstructorAndInit) {
    ...
    block_pool_ = std::make_shared<DeviceBlockPool>(std::make_shared<DeviceBlockPoolConfig>(config));
    ASSERT_NE(block_pool_, nullptr);

    bool init_result = block_pool_->init();
    EXPECT_TRUE(init_result);

    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 1);
}
...
TEST_F(BlockPoolTest, MTPConvertIndexGlobalIdMapping) {
    ...
    auto pool_cfg = rtp_llm::DeviceBlockPoolConfigHelper::createConfig(cache_cfg);
    ...
    block_pool_ = std::make_shared<DeviceBlockPool>(std::make_shared<DeviceBlockPoolConfig>(pool_cfg));
...
TEST_F(BlockPoolTest, SharedPoolMTPLayoutsUseMainBlockNumAfterTpSync) {
    ...
    // Shared default pool follows the main cache_config.physical_block_count after TP sync.
    // MTP sub-config block_num may still contain the pre-sync local value.
    cache_cfg.block_num = 3;

    auto pool_cfg = rtp_llm::DeviceBlockPoolConfigHelper::createConfig(cache_cfg);
    ASSERT_EQ(pool_cfg.physical_block_count, 3u);
    ASSERT_EQ(pool_cfg.memory_layouts.size(), 3u);
    EXPECT_EQ(pool_cfg.memory_layouts[0].block_num, 3u);
    EXPECT_EQ(pool_cfg.memory_layouts[1].block_num, 3u);
    EXPECT_EQ(pool_cfg.memory_layouts[2].block_num, 3u);
...
TEST_F(BlockPoolTest, AllocSingleBlock) {
    ...
    auto blocks = allocateRequestBlocks(1);
    EXPECT_EQ(blocks.size(), 1);
    EXPECT_GE(blocks[0], 0);
    EXPECT_LT(blocks[0], static_cast<BlockIdxType>(config.physical_block_count));
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 2);
}
...
TEST_F(BlockPoolTest, AllocMultipleBlocks) {
    ...
    int  alloc_count = 5;
    auto blocks      = allocateRequestBlocks(alloc_count);
    EXPECT_EQ(blocks.size(), alloc_count);
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - alloc_count - 1);
...
TEST_F(BlockPoolTest, AllocAllBlocks) {
    ...
    auto blocks = allocateRequestBlocks(config.physical_block_count - 1);
    EXPECT_EQ(blocks.size(), config.physical_block_count - 1);
...
TEST_F(BlockPoolTest, AllocMoreThanAvailable) {
    ...
    auto blocks1 = allocateRequestBlocks(5);
    EXPECT_EQ(blocks1.size(), 5);

    auto blocks2 = allocateRequestBlocks(10);
    EXPECT_EQ(blocks2.size(), 0);
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 6);
}
...
TEST_F(BlockPoolTest, FreeBlocks) {
    ...
    auto blocks = allocateRequestBlocks(5);
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 6);
...
TEST_F(BlockPoolTest, FreePartialBlocks) {
    ...
    block_pool_->decRef(partial_blocks);
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 3);
}
...
TEST_F(BlockPoolTest, ReferenceAndFree) {
    ...
    {
        auto blocks = allocateRequestBlocks(3);
    ...
    {
        auto blocks2 = allocateRequestBlocks(3);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 3);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 3);

        block_pool_->incTreeRef(blocks2, BlockTreeRefType::CACHE);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 3);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 3);

        block_pool_->decRef(blocks2);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 3);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks);

        block_pool_->decTreeRef(blocks2, BlockTreeRefType::CACHE);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks);
    }

    {
        auto blocks = allocateRequestBlocks(2);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 2);

        block_pool_->incTreeRef(blocks, BlockTreeRefType::CACHE);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 2);

        block_pool_->incTreeRef(blocks, BlockTreeRefType::LOAD);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->referencedBlocksNum(BlockTreeRefType::LOAD), 2);
        EXPECT_EQ(block_pool_->referencedBlocksNum(), 2);

        block_pool_->decRef(blocks);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->referencedBlocksNum(BlockTreeRefType::LOAD), 2);
        EXPECT_EQ(block_pool_->referencedBlocksNum(), 0);

        block_pool_->decTreeRef(blocks, BlockTreeRefType::LOAD);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks - 2);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks);
        EXPECT_EQ(block_pool_->referencedBlocksNum(BlockTreeRefType::LOAD), 0);
        EXPECT_EQ(block_pool_->referencedBlocksNum(), 0);

        block_pool_->decTreeRef(blocks, BlockTreeRefType::CACHE);
        EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks);
        EXPECT_EQ(block_pool_->availableBlocksNum(), total_blocks);
    }
}
...
TEST_F(BlockPoolTest, AllocZeroBlocks) {
    ...
    auto blocks = allocateRequestBlocks(0);
...
TEST_F(BlockPoolTest, FreeEmptyVector) {
    ...
    block_pool_->decRef(empty_blocks);
    EXPECT_EQ(block_pool_->freeBlocksNum(), config.physical_block_count - 1);
}
...
TEST_F(BlockPoolTest, MixedAllocFreeOperations) {
    ...
    allocated_blocks.push_back(allocateRequestBlocks(2));
    EXPECT_EQ(block_pool_->freeBlocksNum(), 7);

    allocated_blocks.push_back(allocateRequestBlocks(3));
    EXPECT_EQ(block_pool_->freeBlocksNum(), 4);

    block_pool_->decRef(allocated_blocks[0]);
    EXPECT_EQ(block_pool_->freeBlocksNum(), 6);

    allocated_blocks.push_back(allocateRequestBlocks(4));
    EXPECT_EQ(block_pool_->freeBlocksNum(), 2);

    block_pool_->decRef(allocated_blocks[1]);
    block_pool_->decRef(allocated_blocks[2]);
