...
// The former FusedAsyncReadContext exposed separate match/read contexts. The
// block-tree loader exposes the same stages through backend callbacks. Keep the
// original pending, failure and waiter assertions against the production context.
class LegacyAsyncReadContextTest: public ::testing::Test {
protected:
    void SetUp() override {
        coordinator = makeCoordinator(commits, aborts);
        backend     = std::make_shared<ManualBackend>();
        pool        = std::make_shared<TestBlockPool>();
        block       = pool->malloc().value();
        pool->incRef(block);
        initBackend(*backend, pool);
        context = coordinator->create({}, {}, 0, backend, makeRequest(1));
    ...
    void startMatch() {
        context->setMatchCallback([this](LoadAsyncContext& current, size_t matched) {
            // Match has completed, but no read has been submitted yet.
    ...
    void expectWaitCompletes(std::future<void>& waiter) {
        const auto status = waiter.wait_for(std::chrono::seconds(1));
        EXPECT_EQ(status, std::future_status::ready);
        if (status != std::future_status::ready) {
            context->onTaskFail();  // Release the waiter after recording failure.
        }
        waiter.get();
    }
    ...
    std::shared_ptr<LoadContextCoordinator> coordinator;
    std::shared_ptr<ManualBackend> backend;
    std::shared_ptr<TestBlockPool> pool;
    std::shared_ptr<LoadAsyncContext> context;
};

TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_DoneFalse_WhenMatchNotDone) {
    startMatch();
    EXPECT_FALSE(context->done());
    EXPECT_FALSE(context->success());
}

TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_DoneTrue_WhenMatchDoneButFailed) {
    startMatch();
...
TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_DoneFalse_WhenMatchSuccessButReadNotSet) {
    context->setMatchCallback([this](LoadAsyncContext& current, size_t matched) {
...
TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_SuccessDependsOnReadContext) {
    for (bool read_success : {true, false}) {
        SCOPED_TRACE(read_success);
        context = coordinator->create({}, {}, 0, backend, makeRequest(1));
        ASSERT_TRUE(coordinator->registerContext(context));
        startMatch();
        backend->completeMatch(1);
        ...
        backend->completeRead();
        EXPECT_TRUE(context->done());
        EXPECT_EQ(context->success(), read_success);
    }
}

TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_WaitDone_WaitsForLateReadContext) {
    startMatch();
    auto waiter = std::async(std::launch::async, [this] { context->waitDone(); });
    EXPECT_EQ(waiter.wait_for(std::chrono::milliseconds(20)), std::future_status::timeout);
    backend->completeMatch(1);
    EXPECT_TRUE(backend->readPending());
    EXPECT_EQ(waiter.wait_for(std::chrono::milliseconds(20)), std::future_status::timeout);
    backend->completeRead();
    expectWaitCompletes(waiter);
    EXPECT_TRUE(context->done());
    EXPECT_TRUE(context->success());
}

TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_WaitDone_ReturnsWhenLateReadContextSetNull) {
    startMatch();
    auto waiter = std::async(std::launch::async, [this] { context->waitDone(); });
    EXPECT_EQ(waiter.wait_for(std::chrono::milliseconds(20)), std::future_status::timeout);
    // An empty match explicitly omits the read stage in the block-tree API.
...
TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_WaitDone_ReturnsWhenMatchDoneButFailed) {
    startMatch();
    backend->failNextMatch();
    backend->completeMatch(1);
    auto waiter = std::async(std::launch::async, [this] { context->waitDone(); });
...
TEST_F(LegacyAsyncReadContextTest, FusedAsyncReadContext_DoneTrue_WhenReadContextSetNullAfterMatchSuccess) {
    startMatch();
    EXPECT_FALSE(context->done());
    backend->completeMatch(0);
    EXPECT_FALSE(backend->readPending());
...
// The old empty-worker BroadcastResult represented successful work becoming
// available after waiters had started. With deferred block-tree loads, commit
// publishes that work and the operational settlement callback makes it terminal.
// Use that barrier, not an onDone notification, to retain the old wait contract.
struct LegacyMemoryWaitState {
    std::atomic<int> callback_count{0};
    std::atomic<bool> last_ok{false};
};

std::shared_ptr<LoadContextCoordinator>
makeLegacyMemoryWaitCoordinator(const std::shared_ptr<LegacyMemoryWaitState>& state) {
    return std::make_shared<LoadContextCoordinator>(
        [state](const std::shared_ptr<LoadAsyncContext>& context) {
            context->setSettlementReadyCallback([state](const std::shared_ptr<LoadAsyncContext>& ready) {
                const bool ok = ready->aggregateSuccess();
                state->callback_count.fetch_add(1);
                state->last_ok.store(ok);
                EXPECT_TRUE(ready->settle(ok));
            });
            return true;
        },
        [](LoadAsyncContext&) {});
...
TEST(MemoryAsyncContextTest, waitDone_BlocksUntilBroadcastResultSet_ThenCallbackOnce) {
    auto state = std::make_shared<LegacyMemoryWaitState>();
    state->last_ok.store(true);
    auto coordinator = makeLegacyMemoryWaitCoordinator(state);
    ...
    EXPECT_FALSE(context->done());
    EXPECT_EQ(state->callback_count.load(), 0);

    EXPECT_TRUE(context->commit());
    waiter.join();
    EXPECT_TRUE(context->done());
    EXPECT_TRUE(wait_returned.load());
    EXPECT_EQ(state->callback_count.load(), 1);
    EXPECT_TRUE(state->last_ok.load());
    coordinator->shutdown();
}

TEST(MemoryAsyncContextTest, waitDone_IsIdempotent_CallbackOnlyOnce) {
    auto state = std::make_shared<LegacyMemoryWaitState>();
    auto coordinator = makeLegacyMemoryWaitCoordinator(state);
    auto context = coordinator->create({}, {}, 0);
    ASSERT_NE(context, nullptr);
...
TEST(MemoryAsyncContextTest, waitDone_ConcurrentCallersFinalizeOnce) {
    auto state = std::make_shared<LegacyMemoryWaitState>();
    auto coordinator = makeLegacyMemoryWaitCoordinator(state);
    auto context = coordinator->create({}, {}, 0);
    ASSERT_NE(context, nullptr);
    ...
    for (size_t index = 0; index < 8; ++index) {
        waiters.emplace_back([&] { context->waitDone(); });
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    EXPECT_FALSE(context->done());
    EXPECT_EQ(state->callback_count.load(), 0);
