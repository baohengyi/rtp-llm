...
class _TimedGrammarValidator(GrammarValidator):
    ...
    def __init__(
        ...
        super().__init__(tokenizer_info_json, grammar_config, admission_config)
    ...
    def validate_json(self, schema: str | dict, request_id: str = "") -> bool:
        return self._record_validation(
            request_id,
            lambda: super(_TimedGrammarValidator, self).validate_json(
                schema, request_id
            ),
        )
    ...
    def validate_structural_tag(
        ...
        return self._record_validation(
            request_id,
            lambda: super(_TimedGrammarValidator, self).validate_structural_tag(
                payload, request_id
            ),
        )
...
async def _run(args: argparse.Namespace) -> int:
    ...
    validator = _TimedGrammarValidator(
        tokenizer_info_json,
        grammar_config,
        GrammarAdmissionConfig(
            sandbox_pool_size=args.sandbox_pool_size,
        ),
    )
    ...
    try:
        ...
        print(
            ...
            f"status_message={json.dumps(error.get('status_message', ''), ensure_ascii=False)} "
            f"enqueue_called={backend.enqueue_called}"
        )
