...
class Sm120TestEnvironment(unittest.TestCase):
    def test_sm120_and_packed_ue8m0_are_available(self) -> None:
        self.assertEqual(
            torch.cuda.get_device_capability()[0],
            12,
            "SM120 correctness gate was scheduled on a non-SM120 GPU",
        )
        self.assertTrue(
            is_deep_gemm_e8m0_used(),
            "SM120 correctness gate requires DeepGEMM packed UE8M0 scales",
        )
...
class TritonFp8BlockMoeSm120Test(unittest.TestCase):
    ...
    def test_alignment_capacity_exact_mixed_distribution(self) -> None:
        # 25 routes hit expert 0 and one route hits each of experts 1..7.
        # With block_size=16 this is the distribution that exactly consumes
        # the nine-block worst-case allocation for 32 routes and 8 experts.
        flat_ids = torch.cat((torch.zeros(25), torch.arange(1, 8))).to(
            device="cuda", dtype=torch.int64
        )
        sorted_token_ids, expert_ids, padded = moe_align_block_size_torch(
            flat_ids.view(-1, 1), block_size=16, num_experts=8
        )
        padded_count = int(padded.item())
        self.assertEqual(padded_count, sorted_token_ids.numel())
        self.assertEqual(padded_count, 9 * 16)
        self.assertEqual(expert_ids.numel() * 16, sorted_token_ids.numel())
        valid = sorted_token_ids[sorted_token_ids < flat_ids.numel()]
        self.assertEqual(valid.numel(), flat_ids.numel())
        self.assertTrue(
            torch.equal(
                valid.sort().values,
                torch.arange(flat_ids.numel(), device="cuda", dtype=torch.int32),
            )
        )
    ...
    def test_sm120_max_tokens_env_parsing_and_dispatch(self) -> None:
        env_name = "RTP_LLM_SM120_TRITON_FP8_MAX_TOKENS"
        for raw_value, expected in (("0", 0), ("1", 1), ("32", 32)):
            with self.subTest(raw_value=raw_value), patch.dict(
                os.environ, {env_name: raw_value}
            ):
                self.assertEqual(_get_sm120_triton_max_tokens(), expected)

        for raw_value in ("", "not-an-int", "-1", "33"):
            with self.subTest(raw_value=raw_value), patch.dict(
                os.environ, {env_name: raw_value}
            ), self.assertRaisesRegex(ValueError, env_name):
                _get_sm120_triton_max_tokens()
        ...
        self.assertEqual(executor.sm120_triton_max_tokens, 1)
        ...
        for num_tokens, use_triton in ((1, True), (2, False)):
            payload = ExpertForwardPayload(
                expert_x=torch.empty(
                    num_tokens,
                    self.HIDDEN_SIZE,
            ...
            triton_result = object()
            deepgemm_result = object()
            with self.subTest(num_tokens=num_tokens), patch(
                "rtp_llm.models_py.modules.factory.fused_moe.impl.cuda.executors."
                "deepgemm_hybrid_executor._is_cuda_graph_warmup_or_capture",
                return_value=True,
            ), patch.object(
                executor, "execute_triton_fp8", return_value=triton_result
            ) as triton, patch.object(
                executor, "execute_contiguous", return_value=deepgemm_result
            ) as deepgemm:
                result = executor.execute(payload, **call_args)
                self.assertIs(result, triton_result if use_triton else deepgemm_result)
                self.assertEqual(triton.call_count, int(use_triton))
                self.assertEqual(deepgemm.call_count, int(not use_triton))

    def test_noncontiguous_topk_tensors_match_contiguous(self) -> None:
        payload = self._make_payload(offset=0)
        expected = self._triton(payload)
        payload.expert_topk_ids = (
            payload.expert_topk_ids.transpose(0, 1).contiguous().transpose(0, 1)
        )
        payload.expert_topk_weights = (
            payload.expert_topk_weights.transpose(0, 1).contiguous().transpose(0, 1)
        )
        self.assertFalse(payload.expert_topk_ids.is_contiguous())
        self.assertFalse(payload.expert_topk_weights.is_contiguous())
        actual = self._triton(payload)
