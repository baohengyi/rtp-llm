...
class DeepGemmHybridExecutorTestBase:
    ...
    def _run_deepep_normal_executor(
        ...
        cpu_only_expert_num_tokens: bool = False,
    ):
        # generate data
        config = self._generate_config(
            enable_cuda_graph=enable_cuda_graph,
            # CPU-only counts are a contiguous-path contract; the masked
            # path has always required the device-side count tensor.
            force_contiguous=empty_local_experts or cpu_only_expert_num_tokens,
        )
        ...
        if cpu_only_expert_num_tokens:
            self.assertIsNotNone(payload.expert_tokens_meta.expert_num_tokens_cpu)
            payload.expert_tokens_meta.expert_num_tokens = None
        combine_payload = self._execute(
            executor, payload, enable_cuda_graph=enable_cuda_graph
        )
        token_idx = 0
    ...
    def test_cpu_only_expert_num_tokens_eager(self):
        self._run_deepep_normal_executor(cpu_only_expert_num_tokens=True)
