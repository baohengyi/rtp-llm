...
async def fake_output_generator_without_aux_info(
    output_ids: List[int], num_choices: int, input_ids: torch.Tensor
) -> AsyncGenerator[GenerateOutputs, None]:
...
class OpenaiResponseTest(IsolatedAsyncioTestCase):
    ...
    async def _run_without_aux_info(
        self, stream: bool, num_choices: int, termination: str = "eos"
    ):
        ...
        max_tokens = len(output_ids) if termination == "max_tokens" else 100
        expected_reason = FinisheReason.length
        if termination == "eos":
            output_ids.append(renderer.eos_token_id)
            expected_reason = FinisheReason.stop
        elif termination == "max_seq_len":
            renderer.max_seq_len = 7 + len(output_ids)
        ...
        class BackendWithoutAuxInfo:
            ...
            async def enqueue(self, generate_input: GenerateInput):
                ...
                return fake_output_generator_without_aux_info(
                    output_ids, num_choices, generate_input.token_ids.reshape(1, -1)
                )
        ...
        self.assertTrue(
            all(
                choice.finish_reason == expected_reason
        ...
        self.assertEqual(complete_response.usage.prompt_tokens, 7)
        self.assertEqual(
            complete_response.usage.completion_tokens, len(output_ids) * num_choices
        )
        self.assertEqual(
            complete_response.usage.total_tokens, 7 + len(output_ids) * num_choices
        )
        self.assertEqual(chunks[-1].usage, complete_response.usage)
        serialized_chunks = [
    ...
    async def test_aux_info_false_preserves_eos_and_usage(self):
    ...
    async def test_aux_info_false_preserves_length_limits_and_usage(self):
        for termination in ("max_tokens", "max_seq_len"):
            for stream, num_choices in ((True, 1), (False, 1), (True, 3), (False, 3)):
                with self.subTest(
                    termination=termination, stream=stream, num_choices=num_choices
                ):
                    await self._run_without_aux_info(stream, num_choices, termination)
