        ...
        for f in files:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    ...
                    if module.startswith("transformers") or module.startswith(
                        "sentence_transformers"
                    ):
                        for alias in node.names:
                            imports.add((module, alias.name, filepath))
                elif isinstance(node, ast.Import):
                    # bare `import transformers.models.xxx`: attr None = verify module imports
                    for alias in node.names:
                        if alias.name.startswith(
                            "transformers"
                        ) or alias.name.startswith("sentence_transformers"):
                            imports.add((alias.name, None, filepath))
    return imports
...
def _check_imports(imports):
    failures = []
    checked = set()
    # attr_name is None for bare `import x.y` entries; use "" so None and str sort together
    for module_path, attr_name, filepath in sorted(
        imports, key=lambda t: (t[0], t[1] or "", t[2])
    ):
